...
_SHARED_PROMPT_TAIL = (
    ...
    "A line in square brackets starting with 'context:' may come before the "
    "user's words. It is generated by the assistant, not spoken by the user: it "
    "appears only when time has passed or the machine restarted between turns. "
    "Use it to judge whether what you remember is still true -- windows you "
    "opened, commands you ran, things you called current. Do not greet the user "
    "about it or mention the time unless it changes your answer."
)
...
# through two service restarts and a thirteen-hour overnight shutdown without
# anything in the prompt saying so.
#
# So mark the joins, and only the joins. A timestamp on every turn would be
# echoed back ("good evening", "still at it") because anything sitting next to
# the user's words invites acknowledgement; a marker that appears only when
# something changed has nothing to acknowledge on an ordinary turn, and means
# something when it does appear.
CONTEXT_GAP_SECONDS = float(os.getenv("VOICE_ASSISTANT_CONTEXT_GAP", "900"))
CONTEXT_MARKERS = os.getenv("VOICE_ASSISTANT_CONTEXT_MARKERS", "1").strip().lower() \
    not in ("0", "false", "no", "off")
# Identifies this process. A stored id that differs means the assistant was
# restarted between turns, which matters more than the clock: windows the model
# opened are gone, the microphone was reopened, background work died.
RUN_ID = uuid.uuid4().hex


def _boot_id() -> str:
    """Changes on reboot, so it separates "the service restarted" from "the
    machine went down"."""
    try:
...
def _timezone_name() -> str:
    """IANA name where there is one; catches the user having travelled."""
    try:
...
class VoiceAssistant:
    ...
    def _turn_context(self) -> str:
        """One line of background when something changed between turns, else "".

        Always rewrites the stored state, so the gap is measured from the last
        thing the user actually said rather than from the last time a marker
        happened to fire.
        """
        ...
        now = time.time()
        boot, tz = _boot_id(), _timezone_name()
        prev = {}
        try:
            prev = json.loads(TURN_CONTEXT_FILE.read_text())
        except (OSError, ValueError):
            pass

        notes = []
        last = prev.get("at")
        if isinstance(last, (int, float)) and now - last >= CONTEXT_GAP_SECONDS:
            notes.append(f"{_format_gap(now - last)} since the last exchange, "
                         f"now {time.strftime('%a %d %b %H:%M %Z')}")
        # Only against a state file that was actually written by a previous
        # turn: on a first run everything looks like a change.
        if prev:
            if boot and prev.get("boot") and boot != prev["boot"]:
                notes.append("the machine rebooted since then")
            elif prev.get("run") and prev["run"] != RUN_ID:
                notes.append("the assistant restarted since then")
            if tz and prev.get("tz") and tz != prev["tz"]:
                notes.append(f"timezone is now {tz}, was {prev['tz']}")

        try:
            TURN_CONTEXT_FILE.write_text(json.dumps(
                {"at": now, "boot": boot, "run": RUN_ID, "tz": tz}))
        except OSError as e:
            self.logger.debug(f"Could not write {TURN_CONTEXT_FILE}: {e}")
